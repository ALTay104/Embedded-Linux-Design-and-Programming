...
{
    seq_printf(m, "%lu\n", ( ((unsigned long) get_jiffies_64() - INITIAL_JIFFIES) / HZ) );
    return 0;
}
...
{
    proc_create("jiffies_count", 0, NULL, &jiffies_proc_fops);
    return 0;
}
...
{
    remove_proc_entry("jiffies_count", NULL);
}

module_init(jiffies_proc_init);
module_exit(jiffies_proc_exit);

MODULE_AUTHOR("Proc Example");
MODULE_LICENSE("GPL");
